...
class LocationAddressHandler:
    ...
    def handle_manual_address_entry(self, state: dict, original_message: str, session_id: str):
        ...
        if len(address) < 10 or not any(char.isdigit() for char in address) and not any(char.isalpha() for char in address):
            return self.whatsapp_service.create_button_message(
                ...
                "Example: 123 Main St, Wuse 2, Abuja\n\nPlease select an option below:",
                [
                    {"type": "reply", "reply": {"id": "type_address_manually", "title": "✏️ Try Again"}},  # 12 chars
                    {"type": "reply", "reply": {"id": "share_current_location", "title": "📍 Share Location"}}  # 15 chars
                ]
            )
    ...
    def handle_confirm_coordinates(self, state: dict, message: str, session_id: str):
        ...
        if message == "use_coordinates":
            ...
            state["location_coordinates"] = coords
            self._save_address_to_user_details(state, address_fallback, session_id)
            state.pop("temp_coordinates", None)

            maps_link_info = ""
            if self.config.ENABLE_LOCATION_FEATURES and self.location_service.validate_api_key():
                maps_link = self.location_service.generate_maps_link_from_coordinates(coords["latitude"], coords["longitude"])
                maps_link_info = f"\n🗺️ *View on Maps:* {maps_link}"
            ...
            maps_link_info = ""
            if state.get("temp_coordinates") and self.config.ENABLE_LOCATION_FEATURES and self.location_service.validate_api_key():
                maps_link = self.location_service.generate_maps_link_from_coordinates(state["temp_coordinates"]["latitude"], state["temp_coordinates"]["longitude"])
                maps_link_info = f"\n🗺️ *View on Maps:* {maps_link}"
            return self.whatsapp_service.create_button_message(
                session_id,
                f"❌ *Invalid option: '{message}'*\n\n{coordinates_text}{maps_link_info}\n\nPlease select an option below:",
    ...
    def _save_address_to_user_details(self, state: dict, address: str, session_id: str):
        """
        Saves address to data_manager's user_details.
        """
        logger.debug("Saving address '%s' for %s", address, session_id)
        user_data = {
        ...
        try:
            ...
            logger.info("Address saved for %s", session_id)
    ...
    def _proceed_to_order_confirmation(self, state: dict, session_id: str, address: str, maps_info: str = ""):
        ...
        if state.get("from_confirm_order", False):
            ...
            return {
                ...
                "additional_message": f"✅ *Address Updated:*\n\n📍 {address}{maps_info}"
            }
        ...
        state["current_state"] = "confirm_order"
        buttons = [
            {"type": "reply", "reply": {"id": "confirm_order_final", "title": "✅ Confirm Order"}},  # 14 chars
            {"type": "reply", "reply": {"id": "cancel_checkout", "title": "❌ Cancel"}}          # 8 chars
        ]

        cart_summary = format_cart(state["cart"])
        logger.info("Proceeding to order confirmation for %s with address: %s", session_id, address)
        return self.whatsapp_service.create_button_message(
            session_id,
            f"✅ *Address Confirmed!*\n\n📍 *Delivery Address:* {address}{maps_info}\n\n📦 *Order Summary:*\n{cart_summary}\n\nPlease select an option below:",
            buttons
        )
